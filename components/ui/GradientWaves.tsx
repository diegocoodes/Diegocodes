...
export default function GradientWaves({
  ...
  useEffect(() => {
    ...
    const prefersDataSaving = Boolean(
    ...
    enableMouseRef.current = mouseInteraction && hasFinePointer;
    ...
    let webgl2Context: WebGL2RenderingContext | null = null;

    try {
      webgl2Context = canvas.getContext("webgl2", contextAttributes);
    } catch {
      webgl2Context = null;
    }
      ...
      uniforms: {
        ...
        uSteps: {
          value: Math.min(
            ...
            isSmallScreen ? 26 : window.innerWidth < 768 ? 40 : 110
          ),
        },
    ...
    const resizeObserver =
      "ResizeObserver" in window ? new ResizeObserver(setSize) : null;
    ...
    const frameInterval = 1000 / (isSmallScreen ? 15 : 30);
    ...
    const tryStart = () => {
      if (!contextIsLost && isVisible && isPageVisible && animationFrame === 0) {
        animationFrame = window.requestAnimationFrame(loop);
      }
    };
    const tryStop = () => {
